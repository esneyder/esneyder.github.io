...
class InputFilter {
	...
	public function limpiar_uno($source) {//limpiado general de las script que insertan ñlos bandalos
		if (is_array($source)) {
	...
	protected function limpiar_dos($source) {//remover todo los xxs que inserten los banditos
		$loopCounter=0;
	...
	protected function limpiar_etiquetas($source) {//limpiar bichos raros que le inserten a los inputs
		$preTag = NULL;
		$postTag = $source;
	...
	protected function limipiar_atributos($attrSet) {	//limpiar atributos insertados en los inputs
		$newSet = array();
		for ($i = 0; $i <count($attrSet); $i++) {
	...
	protected function limpiado_inteligente($source, &$connection) {
		if (get_magic_quotes_gpc()) $source = stripslashes($source);
		$source = $this->escapeString($source, $connection);
		return $source;
	}
	protected function limpiar_string($string, &$connection) {
